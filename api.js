...
(function () {
  ...
  const remote = {
    ...
    async createJob(fields) {
      return (await post({ action: 'create', job: fields })).job;
    },
  ...
  const DEMO_KEY = 'sns-job-tracker-demo-v1';
  ...
  function demoSeed() {
    ...
    const job = (o) => Object.assign({
      ...
      payment: '', notes: '', createdAt: now, updatedAt: now
    }, o);
    return {
      jobs: [
        ...
        job({ job: 1002, dateIn: isoDay(-10), due: isoDay(-1), customer: 'SAMPLE - Main St Coffee',
          ...
          notes: 'Overdue example - due date is past' }),
      ...
      lists: {
        ...
        options: {
          ...
          artwork: ['Sent to Customer', 'Approved', 'Digitized'],
  ...
  const demo = {
    ...
    async createJob(fields) {
      ...
      const d = db();
      const now = new Date().toISOString();
      const next = d.jobs.reduce((m, j) => Math.max(m, j.job), 1000) + 1;
      const job = Object.assign({ dateIn: isoDay(0), status: 'Active' }, fields,
        { job: next, createdAt: now, updatedAt: now });
      d.jobs.push(job);
      d.activity.push({ job: next, at: now, field: 'Created', oldValue: '', newValue: job.customer + ' - ' + job.type });
      save();
      return copy(job);
    },
    async updateField(jobNo, field, value) {
